...
def register(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(request, f'Аккаунт {username} был успешно создан, введите имя пользователя и пароль для активации')
...
def profile(request):
    if request.method == "POST":
        ...
        if update_user.is_valid() and img_profile.is_valid:
            update_user.save()
            img_profile.save()
            messages.success(request, f'Ваш аккаунт обновлен успешно')
